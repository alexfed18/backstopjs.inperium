...
module.exports.createPlaywrightBrowser = async function (config) {
  ...
  let { engineOptions: sanitizedEngineOptions } = JSON.parse(
  ...
  if (!browserChoice) {
    console.warn(
      chalk.yellow("No Playwright browser specified, assuming Chromium."),
    );
  ...
  if (typeof headless === "string" && headless !== "new") {
    console.warn(
      chalk.yellow(
        `The headless mode, "${headless}", may not be supported by Playwright.`,
      ),
    );
  }
  ...
  if (!playwright[browserChoice]) {
    console.error(
      chalk.red(`Unsupported Playwright browser "${browserChoice}"`),
    );
    return;
  }
  ...
  if (typeof headless !== "undefined" && typeof headless !== "boolean") {
    sanitizedEngineOptions = {
      ...sanitizedEngineOptions,
      ignoreDefaultArgs: sanitizedEngineOptions.ignoreDefaultArgs
        ? [...sanitizedEngineOptions.ignoredDefaultArgs, "--headless"]
        : ["--headless"],
    };
   ...
   * ```
   */
  const playwrightArgs = Object.assign({}, sanitizedEngineOptions, {
...
module.exports.runPlaywright = function ({
  scenario,
  viewport,
  config,
  ...
  config._bitmapsTestPath =
    config.paths.bitmaps_test || DEFAULT_BITMAPS_TEST_DIR;
  config._bitmapsReferencePath =
    config.paths.bitmaps_reference || DEFAULT_BITMAPS_REFERENCE_DIR;
  config._fileNameTemplate =
    config.fileNameTemplate || DEFAULT_FILENAME_TEMPLATE;
  config._outputFileFormatSuffix =
    "." +
    ((config.outputFormat && config.outputFormat.match(/jpg|jpeg/)) || "png");
  config._configId =
    config.id || engineTools.genHash(config.backstopConfigFileName);
...
async function processScenarioView(
  ...
  const ignoreHTTPSErrors = engineOptions.ignoreHTTPSErrors
    ? engineOptions.ignoreHTTPSErrors
    : true;
  const storageState = engineOptions.storageState
    ? engineOptions.storageState
    : {};
  ...
  page.setDefaultNavigationTimeout(
    engineTools.getEngineOption(config, "waitTimeout", TEST_TIMEOUT),
  );
  // Increase default action timeout to avoid screenshot timeouts on heavy pages
  page.setDefaultTimeout(
    engineTools.getEngineOption(config, "actionTimeout", 120000),
  );
  ...
  if (readyEvent) {
    readyPromise = new Promise((resolve) => {
      ...
      readyTimeoutTimer = setTimeout(() => {
        console.error(
          chalk.red(
            `ReadyEvent not detected within readyTimeout limit. (${readyTimeout} ms)`,
          ),
          scenario.url,
        );
        resolve();
  ...
  const playwrightCommands = async () => {
    ...
    if (onBeforeScript) {
      ...
      if (fs.existsSync(beforeScriptPath)) {
        await require(beforeScriptPath)(
          page,
          scenario,
          viewport,
          isReference,
    ...
    const gotoParameters =
      scenario?.engineOptions?.gotoParameters ||
    ...
    await page.evaluate(
      `window._selectorExpansion = '${scenario.selectorExpansion}'`,
    );
    await page.evaluate(`window._backstopSelectors = '${scenario.selectors}'`);
    result = await page.evaluate(() => {
      if (window._selectorExpansion.toString() === "true") {
        window._backstopSelectorsExp = window._backstopTools.expandSelectors(
          window._backstopSelectors,
        );
      } else {
  ...
  await playwrightCommands().catch((e) => {
    console.log(
      chalk.red(
        `Playwright encountered an error while running scenario "${scenario.label}"`,
      ),
    );
...
async function captureScreenshot(
  ...
  let filePath;
  const fullPage =
    selector === NOCLIP_SELECTOR || selector === DOCUMENT_SELECTOR;
  if (selector) {
